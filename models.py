...
users = Table("users", metadata,
    ...
    Column("verified_by_admin", Boolean, default=False),
    Column("admin_verified_at", DateTime, nullable=True),
    Column("otp_verified", Boolean, default=False),
    Column("otp_verified_at", DateTime, nullable=True),
    Column("otp_code", Text, nullable=True),
    Column("otp_expires_at", DateTime, nullable=True),
...
vote_choices = Table("vote_choices", metadata,
    Column("id", UUID(as_uuid=True), primary_key=True,
           default=uuid.uuid4),
    Column("voter_id", UUID(as_uuid=True),
           ForeignKey("users.id", ondelete="CASCADE"),
    ...
    Column("candidate_id", UUID(as_uuid=True),
           ForeignKey("candidates.id", ondelete="CASCADE"),
           nullable=False),
    Column("candidate_name", Text, nullable=False),
    Column("position", Text, nullable=False),
    Column("team_name", Text, nullable=False),
    Column("running_mate", Text, nullable=True),
    Column("voted_at", DateTime, default=sa.func.now()),
)
...
verification_logs = Table("verification_logs", metadata,
    ...
    Column("distance", Float, nullable=True),
       # `selfie_url` removed (OTP/email flow). Use `upload` fields or audit tables if needed.
    Column("created_at", DateTime, default=sa.func.now()),
)
